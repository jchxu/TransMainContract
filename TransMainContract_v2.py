...
Iresfile = open(file='J_MainContract.csv',mode='w')
    ...
    for j in range(1,13):
        ...
        if dt <= '201712':  #指定读取截止时间
            ...
            for index,row in filedata.iterrows():
                ...
                #    RBRec.append(row)
                if re.match(regexI,row.CONTRACTID):
                    IRec.append(row)
            del filename
